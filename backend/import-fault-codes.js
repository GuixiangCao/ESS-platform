...
// CSV文件路径（可通过命令行参数指定）
const CSV_FILE_PATH = process.argv[2] || path.join(__dirname, '..', 'PCS停机故障表 1.csv');
...
function inferDeviceType(category, code) {
  ...
  if (codeUpper.startsWith('BAT_') || category.includes('电池')) return 'battery';
  if (codeUpper.startsWith('AC_') || category.includes('空调')) return 'ac';
  if (codeUpper.startsWith('EMS_') || category.includes('EMS')) return 'ems';
  if (codeUpper.startsWith('SYS_') || category.includes('系统') || category.includes('环境')) return 'system';
 ...
 * 从CSV导入故障代码
 */
async function importFaultCodesFromCSV() {
  let connection;
  ...
  try {
    ...
    console.log(`正在读取CSV文件: ${CSV_FILE_PATH}\n`);
      ...
      fs.createReadStream(CSV_FILE_PATH)
        ...
        .on('data', (row) => {
          ...
          try {
            ...
            const categoryKey = keys.find(k => k.includes('分类') || k === 'category');
            const codeKey = keys.find(k => k.includes('英文') || k.includes('名称') || k === 'code');
            const descKey = keys.find(k => k.includes('中文') || k.includes('描述') || k === 'description');
        ...
        .on('end', async () => {
          try {
            console.log(`CSV文件读取完成，共读取 ${lineNumber} 行数据`);
...
if (require.main === module) {
  console.log('💡 使用方法: node import-fault-codes.js [CSV文件路径]');
  console.log(`📁 当前使用文件: ${CSV_FILE_PATH}\n`);

  importFaultCodesFromCSV()
    .then(() => {
      console.log('\n🎉 导入完成!');
      process.exit(0);
    })
...
module.exports = { importFaultCodesFromCSV };
